...
void SharedResourceManager::Init()
{
	...
	m_sceneInstanceSSBOLayout = DescriptorLaytoutUtils::CreateOneDescriptorSetForAll(
		{
			PipelineDescriptorLayout(UBO::BufferType::TransformSSBO), PipelineDescriptorLayout(UBO::BufferType::GlobalObjectDataSSBOs), PipelineDescriptorLayout(UBO::BufferType::InstanceDataSSBO)
		}
	);
	...
	for (u32 i = 0; i < FRAMES_IN_FLIGHT; ++i)
	{
		...
		m_frameData[i].pSceneInstanceSSBOSet->SetBindingSlot(UBO::s_UBOTypeToBindingSlot[UBO::BufferType::TransformSSBO]);
		{
...
void SharedResourceManager::UploadSceneGeometry(const stltype::vector<stltype::unique_ptr<Mesh>>& meshes)
{
	...
	for (const auto& pMesh : meshes)
	{
		...
		Utils::GenerateDrawCommandForMesh(*pMesh.get(), m_bufferOffsetData.indexBufferOffset, cmd.vertices, cmd.indices);
...
void SharedResourceManager::UpdateInstanceDataSSBO(stltype::vector<RenderPasses::PassMeshData>& meshes, u32 thisFrameNum)
{
...
void SharedResourceManager::WriteInstanceSSBODescriptorUpdate(u32 targetFrame)
{
	...
	m_frameData[targetFrame].pSceneInstanceSSBOSet->WriteSSBOUpdate(m_sceneInstanceBuffer, s_globalInstanceDataSSBOSlot);
	m_frameData[targetFrame].pSceneInstanceSSBOSet->WriteSSBOUpdate(m_materialBuffer, s_globalMaterialBufferSlot);
}

void SharedResourceManager::UpdateTransformBuffer(const stltype::vector<DirectX::XMFLOAT4X4>& transformBuffer, u32 thisFrame)
{
	auto pDescriptor = m_frameData[thisFrame].pSceneInstanceSSBOSet;
	AsyncQueueHandler::SSBOTransfer transfer{
		.data = (void*)transformBuffer.data(),
