...
struct ImageMemoryBarrier
{
	...
	ImageMemoryBarrier(MemoryBarrierType t, StageFlags src, StageFlags dst, AccessFlags srcAccess, AccessFlags dstAccess,
		AspectFlags aspect, u32 baseMip, u32 levelCnt, u32 baseLayer, u32 layerCnt)
		: type(t), srcStage(src), dstStage(dst), srcAccessMask(srcAccess), dstAccessMask(dstAccess),
		aspectMask(aspect), baseMipLevel(baseMip), levelCount(levelCnt), baseArrayLayer(baseLayer), layerCount(layerCnt)
	{
	}
	...
	template<>
	static ImageMemoryBarrier FromTemplate<MemoryBarrierTemplate::ColorAttachmentReadWrite>(MemoryBarrierTemplate barrierTemplate);
};
