...
struct PipelineDescriptorLayout
{
    ...
    PipelineDescriptorLayout(
        UBO::BufferType typeTemplate,
        u32 setIdx = 0) // setIdx can be left empty if this just used to simplify layout creation for descriptorpools
    {
    ...
    PipelineDescriptorLayout(Bindless::BindlessType typeTemplate, u32 setIdx = 0)
        : PipelineDescriptorLayout(typeTemplate, ShaderTypeBits::All, setIdx)
    {
    }
    ...
    inline bool IsBindless() const
    {
        return Bindless::IsBindless(type);
    }
};