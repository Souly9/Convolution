...
// ============================================================================
// DLSSRRPass Implementation
// ============================================================================

DLSSRRPass::DLSSRRPass() : ConvolutionRenderPass("DLSSRRPass")
{
}

DLSSRRPass::~DLSSRRPass()
{
}
...
void DLSSRRPass::Render(const MainPassData& data, FrameRendererContext& ctx, CommandBuffer* pCmdBuffer)
{
    ...
    const bool shouldReset =
        !m_wasActive || data.renderState.recreatedThisFrame || Nvidia::StreamlineManager::ConsumeDLSSDResetFlag();
    ...
    streamlineCmd.callback = [tagDescs, pFrameToken, frameSlot](void* pNativeCmdBuf) mutable {
        ...
        {
            DEBUG_LOG_WARNF("[DLSSRRPass] slSetTagForFrame failed with result: 0x{:X}", static_cast<u32>(tagRes));
            return;
        }
