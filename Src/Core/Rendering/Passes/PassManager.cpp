...
void RenderPasses::PassManager::CreatePassObjectsAndLayouts()
{
	...
	m_viewUBOLayout = DescriptorLaytoutUtils::CreateOneDescriptorSetForAll({ PipelineDescriptorLayout(UBO::BufferType::View)});
	m_gbufferPostProcessLayout = DescriptorLaytoutUtils::CreateOneDescriptorSetForAll({ PipelineDescriptorLayout(UBO::BufferType::GBufferUBO), PipelineDescriptorLayout(UBO::BufferType::ShadowmapUBO) });
	m_shadowViewUBOLayout = DescriptorLaytoutUtils::CreateOneDescriptorSetForAll({ PipelineDescriptorLayout(UBO::BufferType::ShadowmapViewUBO) });
}
...
void RenderPasses::PassManager::InitPassesAndImGui()
{
	...
	VkDescriptorSet depthId = ImGui_ImplVulkan_AddTexture(
		pDepthTex->GetSampler(),
		pDepthTex->GetImageView(),
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	);
	VkDescriptorSet csmId = ImGui_ImplVulkan_AddTexture(
		m_globalRendererAttachments.directionalLightShadowMap.pTexture->GetSampler(),
		m_globalRendererAttachments.directionalLightShadowMap.pTexture->GetImageView(),
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	);
	VkDescriptorSet id1 = ImGui_ImplVulkan_AddTexture(
		m_gbuffer.Get(GBufferTextureType::GBufferNormal)->GetSampler(),
		m_gbuffer.Get(GBufferTextureType::GBufferNormal)->GetImageView(),
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	);
	VkDescriptorSet id2 = ImGui_ImplVulkan_AddTexture(
		m_gbuffer.Get(GBufferTextureType::GBufferAlbedo)->GetSampler(),
		m_gbuffer.Get(GBufferTextureType::GBufferAlbedo)->GetImageView(),
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	);
	VkDescriptorSet id3 = ImGui_ImplVulkan_AddTexture(
		m_gbuffer.Get(GBufferTextureType::TexCoordMatData)->GetSampler(),
		m_gbuffer.Get(GBufferTextureType::TexCoordMatData)->GetImageView(),
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	);
	VkDescriptorSet id4 = ImGui_ImplVulkan_AddTexture(
		m_gbuffer.Get(GBufferTextureType::GBufferUI)->GetSampler(),
		m_gbuffer.Get(GBufferTextureType::GBufferUI)->GetImageView(),
		VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
	);
	g_pApplicationState->RegisterUpdateFunction([depthId, csmId, id1, id2, id3, id4](auto& state)
		{
...
void RenderPasses::PassManager::PrepareMainPassDataForFrame(MainPassData& mainPassData, FrameRendererContext& ctx, u32 frameIdx)
{
...
void RenderPasses::PassManager::PerformInitialLayoutTransitions(FrameRendererContext& ctx, const stltype::vector<const Texture*>& gbufferTextures, Texture* pSwapChainTexture, Semaphore& imageAvailableSemaphore)
{
...
void RenderPasses::PassManager::RenderAllPassGroups(const MainPassData& mainPassData, FrameRendererContext& ctx, const stltype::hash_map<ConvolutionRenderPass*, RenderPassSynchronizationContext>& syncContexts, Semaphore& imageAvailableSemaphore)
{
...
void RenderPasses::PassManager::ExecutePasses(u32 frameIdx)
{
	...
	AsyncQueueHandler::PresentRequest presentRequest{
		.pWaitSemaphore = &ctx.additionalSynchronizationContexts[2].signalSemaphore,
...
void RenderPasses::PassManager::PreProcessDataForCurrentFrame(u32 frameIdx)
{
	...
	if (m_needsToPropagateMainDataUpdate)
	{
		...
		if (m_dataToBePreProcessed.IsEmpty())
		{
			...
			if (m_dataToBePreProcessed.mainViewUBO.has_value() == false)
			{
	...
	if(m_dataToBePreProcessed.IsEmpty() == false)
	{
		...
		// Light uniforms
		{
			...
			mainPassData.mainView.viewport = RenderViewUtils::CreateViewportFromData(FrameGlobals::GetSwapChainExtent(), camComp->zNear, camComp->zFar);
		...
		if (m_dataToBePreProcessed.lightVector.empty() == false || m_dataToBePreProcessed.dirLightVector.empty() == false)
		{
			// Light data
			...
			if (m_dataToBePreProcessed.dirLightVector.empty() == false)
			{
				...
				tileArray.dirLight.direction = mathstl::Vector4(dirLight.direction.x, dirLight.direction.y, dirLight.direction.z, 1);
...
{
	DispatchSSBOTransfer((void*)&data.dirLight, m_frameRendererContexts[frameIdx].tileArraySSBODescriptor, sizeof(data.dirLight), &m_tileArraySSBO, 0, s_tileArrayBindingSlot);
	DispatchSSBOTransfer((void*)data.tiles[0].lights.data(), m_frameRendererContexts[frameIdx].tileArraySSBODescriptor, UBO::GlobalTileArraySSBOSize - sizeof(data.dirLight), &m_tileArraySSBO, sizeof(data.dirLight), s_tileArrayBindingSlot);
}
