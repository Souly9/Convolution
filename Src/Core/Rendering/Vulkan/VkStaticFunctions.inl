...
namespace SRF
{
		...
		QueueType queue)
	{
        ...
        auto FillSemaphoreSubmitInfo = [](u32& infoCount, stltype::vector<VkSemaphoreSubmitInfo>& infoVector, const stltype::vector<RawSemaphoreHandle>& semaphores, u32 stageMask)
            {
        ...
        for (const auto* pCmdBuffer : commandBuffers)
        {
            ...
            submitInfo.pCommandBufferInfos = &commandBufferInfo; // Pointers to the structs you've filled
            ...
            if (waitSemaphores.empty() == false)
            {
                ...
                if (addedSemaphores > 0)
                {
                    ...
                    submitInfo.pWaitSemaphoreInfos = &waitSemaphoreInfos.at(waitSemaphoreInfos.size() - addedSemaphores);
                }
            }
            else
            {
            ...
            if (signalSemaphores.empty() == false)
            {
                ...
                if (addedSemaphores > 0)
                {
                    ...
                    submitInfo.pSignalSemaphoreInfos = &signalSemaphoreInfos.at(signalSemaphoreInfos.size() - addedSemaphores);
                }
            }
            else
            {
	...
	template<>
	static inline void QueryImageForPresentationFromMainSwapchain<Vulkan>(const Semaphore& imageAvailableSemaphore, const Fence& imageAvailableFence, u32& imageIndex, u64 timeout)
	{
        VkResult rslt;
        do
        {
            // The timeout should typically be large (e.g., a billion nanoseconds or UINT64_MAX for infinite)
            rslt = vkAcquireNextImageKHR(VkGlobals::GetLogicalDevice(), VkGlobals::GetMainSwapChain(), timeout, imageAvailableSemaphore.GetRef(), imageAvailableFence.GetRef(), &imageIndex);
