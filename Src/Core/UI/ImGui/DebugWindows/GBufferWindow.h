...
public:
    ...
    void DrawWindow(f32 dt)
    {
        ...
        stltype::vector<OtherBuffer> otherBuffers =
        {
            //{ "Depth Buffer", renderState.depthbufferImGuiID },
        ...
        if (otherBuffersCount > 0)
        {
            ...
            if (ImGui::BeginTable("OtherBuffersRow", otherBuffersCount, ImGuiTableFlags_SizingStretchSame | ImGuiTableFlags_Borders))
            {
